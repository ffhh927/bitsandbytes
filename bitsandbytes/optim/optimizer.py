...
class Optimizer8bit(torch.optim.Optimizer):
    ...
    def get_config(self, gindex, pindex, group):
        ...
        config["kl_clip"] = self.args.kl_clip
...
class Optimizer3State(Optimizer8bit):
    def __init__(
        self,
        optimizer_name,
        params,
        ...
        if not 0.0 <= weight_decay:
            raise ValueError(f"Invalid weight_decay value: {weight_decay}")
        defaults = dict(lr=lr, betas=betas, eps=eps, weight_decay=weight_decay)
        super().__init__(params, defaults, optim_bits, False)
    ...
    @torch.no_grad()
    def init_state(self, group, p, gindex, pindex, a=None, g=None,absmax1=None,absmax2=None):
        print(a)
        print(g)
        print(absmax1)
        config = self.get_config(gindex, pindex, group)
        ...
        if dtype == torch.float32:
            state["state1"] = a
            state["state2"] = g
            state["state3"] = self.get_state_buffer(p, dtype=torch.float32)
        elif dtype == torch.uint8:
            if state["step"] == 0:
                if "dynamic" not in self.name2qmap:
                    self.fill_qmap()
            ...
            state["state2"] = g
            state["qmap2"] = self.name2qmap["dynamic"]

            state["state3"] = self.get_state_buffer(p, dtype=torch.uint8)
            state["qmap3"] = self.name2qmap["dynamic"]
            ...
            if config["block_wise"]:
                ...
                state["absmax1"] = absmax1
                state["absmax2"] = absmax2
                state["absmax3"] = torch.zeros((blocks,), dtype=torch.float32, device=p.device)
            else:
                state["max1"] = torch.zeros((1,), dtype=torch.float32, device=p.device)
                state["new_max1"] = torch.zeros((1,), dtype=torch.float32, device=p.device)
                state["max2"] = torch.zeros((1,), dtype=torch.float32, device=p.device)
                state["new_max2"] = torch.zeros((1,), dtype=torch.float32, device=p.device)
                state["max3"] = torch.zeros((1,), dtype=torch.float32, device=p.device)
                state["new_max3"] = torch.zeros((1,), dtype=torch.float32, device=p.device)
...
#     qmap3: Optional[torch.Tensor],
#     absmax1: Tensor,
#     absmax2: Optional[torch.Tensor],
#     absmax3: Optional[torch.Tensor],
...
# )
            ...
            print(f"State1: {state['state1']}, shape: {state['state1'].shape}")
            print(f"State2: {state['state2']}, shape: {state['state2'].shape}")
            print(f"State3: {state['state3']}, shape: {state['state3'].shape}")
            print(f"Beta1: {config['betas'][0]}")
            print(f"Beta2: {config['betas'][1]}")
            print(f"Epsilon: {config['eps']}")
            print(f"Step: {step}")
            print(f"Learning Rate: {config['lr']}")
            print(f"QMap1: {state['qmap1']}, shape: {state['qmap1'].shape}")
            print(f"QMap2: {state['qmap2']}, shape: {state['qmap2'].shape}")
            print(f"QMap3: {state['qmap3']}, shape: {state['qmap3'].shape}")
            print(f"AbsMax1: {state['absmax1']}, shape: {state['absmax1'].shape}")
            print(f"AbsMax2: {state['absmax2']}, shape: {state['absmax2'].shape}")
            print(f"AbsMax3: {state['absmax3']}, shape: {state['absmax3'].shape}")
            print(f"Weight Decay: {config['weight_decay']}")
            print(f"GNorm Scale: {gnorm_scale}")
            print(f"Skip Zeros: {config['skip_zeros']}")
            print(f"KL Clip: {config['kl_clip']}")
            F.optimizer_update_8bit_blockwise(
                self.optimizer_name,
                grad,
                p,
